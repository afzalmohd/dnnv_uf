...
def show_tensor_images(image_tensor, labels=None, num_images=100, size=(1, 28, 28), nrow=10, show=False, save_path=None, csv_path=None):
    ...
    if csv_path:
        flattened_images = image_unflat.view(num_images, -1).numpy()
        if labels is not None:
            labels = labels[:num_images].detach().cpu().numpy()
        else:
            labels = [None] * num_images  
        
        data_with_labels = [[labels[i]] + list(flattened_images[i]) for i in range(num_images)]
