...
def get_fc_layer_weights_top_k_relaxed(top_k_labels, output_dims):
    top1 = top_k_labels[0]
    weights = []
    ...
    for i in range(output_dims):
        if i not in top_k_labels:
            l = [0.0]*output_dims
            l[i] = 1.0
            l[top_k_labels[0]] = -1.0
            weights.append(l)

            l = [0.0]*output_dims
            l[i] = 1.0
            l[top_k_labels[1]] = -1.0
            weights.append(l)


    # print(np.array(weights).reshape(25,10))
...
def get_top_k_preds(net_prp, file_path):
    try:
        with open(file_path, 'r') as f:
            ...
            for line in Lines:
                ...
                if net_prp[0] == line_l[0] and net_prp[1] == line_l[1]:
                    return [int(line_l[3]), int(line_l[4])]
    except:
        return None
...
def setup_on_vnncomp_prop_top_k_relaxed(dataset, timeout, epsilons, target_benchmarks_dir, vnncomp_benchmarks_dir, tolerance_param, conf_file):
    k=2
    ...
    with open(vnncomp_instance_file, 'r') as vnncomp_instance_f:
        ...
        for line in vnncomp_instance_f:
            ...
            update_fc_relu_top_k_relaxed(model_path=vnncomp_net_path, output_model_path=target_net_path, top_k_labels=top_pred_label, existing_model_out_dims=output_dims, k=k)
...
if __name__ == '__main__':
    ...
    update_fc_relu_top_k_relaxed(input_model_path, output_model_path)
    # get_fc_layer_weights_top_k_relaxed([1,4], 10)