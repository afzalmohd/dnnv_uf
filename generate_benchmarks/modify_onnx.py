...
def get_fc_layer_weights(label, output_dims=10, is_target_prop=False):
...
def get_output_layer_weight(output_dim = 10, is_target_prop=False):
    weights = []
    if is_target_prop:
        weights = [-1.0]*(output_dim-1)
    else:
...
def update_fc_relu_softmax_vgg(model_path, output_model_path, label = 0, delta=1.98, fc_output_dim=81, existing_model_out_dims = 10, is_target_prop = False):
    ...
    if is_target_prop:
        ...
        final_out_dim = existing_model_out_dims - 1
        
    new_w = get_fc_layer_weights(label, output_dims=existing_model_out_dims, is_target_prop=is_target_prop)
    ...
    # for initializer in graph.initializer:
    #     if w_weight_name in  initializer.name:
    #         initializer = combined_weight
    #         initializer.dims[:] = combined_weight.shape
    #     elif b_weight_name in  initializer.name:
    #         initializer = combined_bias
    ...
    for initializer in graph.initializer:
        if w_weight_name in initializer.name:
            initializer.CopyFrom(
                numpy_helper.from_array(combined_weight, initializer.name)
            )
            w_name = initializer.name
        elif b_weight_name in initializer.name:
            initializer.CopyFrom(
                numpy_helper.from_array(combined_bias, initializer.name)
            )
            b_name = initializer.name
    # Save the modified model

    for input_tensor in model.graph.input:
        if input_tensor.name == w_name:
            input_tensor.type.tensor_type.shape.dim[0].dim_value = combined_weight.shape[0]
        if input_tensor.name == b_name:
            input_tensor.type.tensor_type.shape.dim[0].dim_value = combined_weight.shape[0]
    ...
    weight = get_output_layer_weight(output_dim=existing_model_out_dims, is_target_prop=is_target_prop)
    ...
    graph.input.append(helper.make_tensor_value_info(weight_name, TensorProto.FLOAT, [final_out_dim, fc_output_dim]))
    graph.input.append(helper.make_tensor_value_info(bias_name, TensorProto.FLOAT, [final_out_dim]))
...
def update_fc_relu_softmax_testing(model_path, output_model_path, label = 0, delta=1.98, fc_output_dim=81, existing_model_out_dims = 10, is_target_prop = False):
    is_vgg_net = True
    w_weight_name, b_weight_name  = get_output_affine_layers_weights(model_path)
    model = onnx.load(model_path)
    graph = model.graph
...
def append_fc_tsr(model_path, output_model_path, label, delta, fc_output_dim, existing_model_out_dims):
    model = onnx.load(model_path)
    graph = model.graph
    ...
    for node in graph.node:
        if node.op_type == "Softmax":
            softmax_node = node
    if not softmax_node:
        raise ValueError("Softmax layer not found in the model.")


    # Remove the softmax layer
    graph.node.remove(softmax_node)

    # Locate the second last MatMul layer
    second_last_matmul = None
    for node in graph.node:
        if node.op_type == "MatMul":
            second_last_matmul = node  # Assuming it's the second last; verify in practice

    if not second_last_matmul:
        raise ValueError("Second last MatMul layer not found.")

    # Get its weight tensor
    second_last_weight_name = second_last_matmul.input[1]
    second_last_weight_tensor = None
    for initializer in graph.initializer:
        if initializer.name == second_last_weight_name:
            second_last_weight_tensor = numpy_helper.to_array(initializer)

    if second_last_weight_tensor is None:
        raise ValueError(f"Weight tensor '{second_last_weight_name}' not found.")

    # Define the new weight tensor for the additional MatMul
    new_w = get_fc_layer_weights(label, output_dims=existing_model_out_dims)
    new_fc_weight = np.reshape(new_w, (fc_output_dim, existing_model_out_dims))
    new_fc_weight = new_fc_weight.T
    new_fc_weight = np.asarray(new_fc_weight, dtype=np.float32)
    new_fc_bias = np.array([delta]*fc_output_dim, dtype=np.float32)
    # Combine the weights and biases
    merged_weight_tensor = np.dot(second_last_weight_tensor, new_fc_weight)

    # Create a new weight tensor initializer
    merged_weight_name = "merged_weight"
    merged_weight_initializer = numpy_helper.from_array(merged_weight_tensor, name=merged_weight_name)

    # Replace the second last MatMul with the merged weight
    second_last_matmul.input[1] = merged_weight_name
    # Add the merged weight tensor to initializers
    graph.initializer.append(merged_weight_initializer)



    new_bias_name = "new_bias"
    new_bias_initializer = numpy_helper.from_array(new_fc_bias, name=new_bias_name)

    # Add the new weight initializer to the model
    graph.initializer.append(new_bias_initializer)

    # Define a new MatMul node
    new_bias_output = "new_bias_output"
    new_bias_node = helper.make_node(
        "Add",
        inputs=[second_last_matmul.output[0], new_bias_name],
        outputs=[new_bias_output],
        name="NewBias"
    )
    graph.node.append(new_bias_node)

    # Add a Relu node after the merged MatMul
    relu_output = "relu_output"
    relu_node = helper.make_node(
        "Relu",
        inputs=[new_bias_output],
        outputs=[relu_output],
        name="MergedRelu"
    )
    ...
    weight_name = f"appended.weight"
    # bias_name = f"layer.appended.bias"
    new_matmul_output = "new_matmul_output"
    fc_node = helper.make_node('MatMul', inputs=[relu_output, weight_name], 
                               outputs=[new_matmul_output],
                               name="NewMatMul"
                               )


    weight = get_output_layer_weight(output_dim=existing_model_out_dims)
    weight = np.array(weight, np.float32).reshape(existing_model_out_dims-1, fc_output_dim)
    weight = weight.T
    fc_weight = helper.make_tensor(name=weight_name, data_type=TensorProto.FLOAT, dims=[fc_output_dim, existing_model_out_dims-1],vals=weight)

    # Add the new weight initializer to the model
    graph.initializer.append(fc_weight)

    # Update the graph output to use the Relu node's output
    graph.node.append(fc_node)
    graph.output[0].name = new_matmul_output
...
def append_layers_softmax(model_path, output_model_path, label = 0, conf=60, fc_output_dim=81, existing_model_out_dims = 10, is_target_prop=False):
    ...
    elif existing_model_out_dims == 1000:
        update_fc_relu_softmax_vgg(model_path, output_model_path, label=label, delta=delta, fc_output_dim=fc_output_dim, 
                               existing_model_out_dims=existing_model_out_dims, is_target_prop=is_target_prop)
    else:
...
def append_layers_vnncomp_prop(input_net_path, target_net_path, conf, orig_label, existing_output_dim=10, dataset='MNIST', is_target_prop = False):
    if conf == 0:
        shutil.copy2(input_net_path, target_net_path)
    else:
        if dataset == 'TSR':
            delta = get_delta(conf)
            append_fc_tsr(input_net_path, target_net_path, label=orig_label, delta=delta, fc_output_dim=(existing_output_dim-1)*(existing_output_dim-1), existing_model_out_dims=existing_output_dim)
        else:
            append_layers_softmax(input_net_path, target_net_path, label=orig_label, conf=conf, existing_model_out_dims=existing_output_dim, is_target_prop=is_target_prop)
...
if __name__ == '__main__':
    ...
    model_path = '/home/u1411251/tools/networks_all/vnncomp2023_benchmarks/benchmarks/traffic_signs_recognition/onnx/3_30_30_QConv_16_3_QConv_32_2_Dense_43_ep_30.onnx'
    output_model_path = 'temp.onnx'
    append_fc_tsr(model_path=model_path, output_model_path=output_model_path, label=0, delta=0.432, fc_output_dim=1764, existing_model_out_dims=43)
