...
import math
import re
from generate_benchmarks.modify_onnx import append_layers, append_layers_vnncomp_prop, append_layers_mod_prop
from generate_benchmarks.generate_properties import gen_props, save_vnnlib_from_vnncomp
from generate_benchmarks.generate_instance_file import gen_instances_file, gen_instances_file_top_k
from generate_benchmarks.simulate_network import get_mnist_test_data, get_mnist_train_data, run_network_mnist_test, select_images_top_k, get_selected_images_gans, get_selected_images_gans_with_delta_th
from generate_benchmarks.simulate_network import get_cifar10_test_data, get_cifar10_train_data, run_network_cifar10, run_model
from generate_benchmarks.modify_onnx_top_k import append_layers_top_k
...
def setup_on_vnncomp_prop(dataset, confs, timeout, epsilons, target_benchmarks_dir, vnncomp_benchmarks_dir, tolerance_param, is_less_than_output_prp, is_target_prop=False):
    ...
    with open(vnncomp_instance_file, 'r') as vnncomp_instance_f:
        ...
        for line in vnncomp_instance_f:
            ...
            for conf in confs:
                ...
                print(f"{os.path.join(sub_net_dir, new_net_name)},{os.path.join(sub_prp_dir, new_prp_name)},{timeout}")
                append_layers_vnncomp_prop(input_net_path=vnncomp_net_path, target_net_path=target_net_path, conf=conf, orig_label=label, existing_output_dim=output_dims, dataset=dataset, is_target_prop=is_target_prop)
...
if __name__ == '__main__':
    # conf_delta()
    # exit(0)
    potential_datasets = [mnist_dataset, cifar10_dataset, cifar100_dataset, tsr_dataset, imagenet_dataset]
    if len(sys.argv) > 1:
