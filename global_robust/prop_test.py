...
def write_vnnlib(lb, ub, label1=0, label2=0, spec_path='prp.vnnlib', dataset='MNIST', total_output_class=10, eta=-1e-3):
...
def generate_prop():
    ...
    lb = [0.0]*2*input_dim
    ub = [1.0]*2*input_dim
    write_vnnlib(lb, ub, eta=-0.1, total_output_class=10, spec_path='prp_mnist.vnnlib') # eta < 1e-4
